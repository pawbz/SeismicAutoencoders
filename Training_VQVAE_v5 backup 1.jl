...
# ╔═╡ 74d0f419-79d0-4a6a-ae65-18f6be9d64d
begin
    @bind selected_pair_name confirm(Select(selected_training_pair_names, default=first(selected_training_pair_names)))
end

# ╔═╡ 74d0f419-79d0-4a6a-ae65-18f6be9d64e
selected_pair = let
...
vqvae_parameters = (;
    ...
    pair_smoothing_mode=:within_pair_only,
...
For `v5`, the important training knobs are:

- `per_pair_batchsize`: smoothing-quality knob. Keep this at least as large as `smooth_max_neighbors`.
...
1. choose `smooth_max_neighbors`
2. set `per_pair_batchsize >= smooth_max_neighbors`
3. choose the largest `pairs_per_batch` that fits on the GPU

Inference should also be done pair-by-pair, or at least with enough same-pair waveforms in the batch for the within-pair coarse smoothing to stay training-like.
"""
...
training_para = let
    pairs_per_batch = min(length(selected_pairs), 4)  # main GPU-memory knob
    per_pair_batchsize = vqvae_para.smooth_max_neighbors  # must be >= smooth_max_neighbors
    vqvae.VQVAE_Training_Para(
        ...
        per_pair_batchsize=per_pair_batchsize,
...
# ╔═╡ 1ddfb196-7d6a-44bf-82fc-793a9a30fb9ca
md"""
...
# ╔═╡ 1ddfb196-7d6a-44bf-82fc-793a9a30fb9cb
analysis_cache_for_disk = @use_memo([trained, cache_pairs_per_batch, selected_training_pair_names, reload_network]) do
    ...
    pair_metadata = [
        (;
            pair_id=pid,
            pair=pair_splits[pid].pair,
            pair_name=join(pair_splits[pid].pair, "-"),
    ...
    (;
        cache_version="vqvae_v5_analysis_cache",
        ...
        vqvae_para=NamedTuple(pairs(vqvae_para)),
        training_para=NamedTuple(pairs(training_para)),
        loss_history,
...
begin
    ...
    cluster_avg_c = cluster_averages_from_codes(data_cpu_cache.D_c, encoded_cache.ci_c;
        K=model.per_position ? vqvae_para.K : vqvae_para.Ksmall,
        per_position=model.per_position)
    cluster_avg_ac = cluster_averages_from_codes(data_cpu_cache.D_ac, encoded_cache.ci_ac;
        K=model.per_position ? vqvae_para.K : vqvae_para.Ksmall,
        per_position=model.per_position)
end
...
The heavy post-training analysis workflow has been moved to:

`/mnt/NAS/EQData/SeismicAutoencoders/Analysis_VQVAE_v5.jl`
...
Open `Analysis_VQVAE_v5.jl` to load `analysis_cache.jld2`, load raw data on CPU, and run single-pair MFT / source-state analysis.
"""
...
begin
	...
	run_name = "vqvae_v5_run_" * run_tag
...
per_position_diag = let
    ...
    if model.per_position
        vqvae.per_position_prototype_snr(model, data.D_all, vqvae_para, data.pair_ids_all)
    else
        nothing
    end
end
