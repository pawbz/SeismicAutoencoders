...
function update(model, ps, st, loss_history, train_data, test_data,
    ...
    for epoch in 1:training_para.nepoch
        ...
        if !isnothing(training_para.stop_on_recon_loss) && train_m.recon_loss < training_para.stop_on_recon_loss
            @info "Early stopping" epoch train_target_mse=train_m.recon_loss threshold=training_para.stop_on_recon_loss
            break
        end
    end
...
function save_vqvae_run(run_dir; model, ps, st, para, training_para, loss_history, pair, data_bundle,
    ...
    psd_dt = hasproperty(analysis_settings, :dt) ? Float64(analysis_settings.dt) : 1.0
    state_psd_ac = _positive_period_psd(averages.acausal, psd_dt)
    state_psd_c = _positive_period_psd(averages.causal, psd_dt)
    global_psd_ac = _positive_period_psd(global_avg_ac, psd_dt)
    global_psd_c = _positive_period_psd(global_avg_c, psd_dt)
    window_psd_ac = _positive_period_psd(data_bundle.D1fac, psd_dt)
    window_psd_c = _positive_period_psd(data_bundle.D1fc, psd_dt)
    state_window_psd_ac = _state_window_mean_psd(window_psd_ac.psd, source_state_ac, size(averages.acausal, 2))
    state_window_psd_c = _state_window_mean_psd(window_psd_c.psd, source_state_c, size(averages.causal, 2))
    raw_stage = _bundle_stage_waveforms(data_bundle, :raw)
    whitened_stage = _bundle_stage_waveforms(data_bundle, :whitened)
    despiked_stage = _bundle_stage_waveforms(data_bundle, :despiked)
    raw_psd_ac = _positive_period_psd(raw_stage.ac, psd_dt)
    raw_psd_c = _positive_period_psd(raw_stage.c, psd_dt)
    whitened_psd_ac = _positive_period_psd(whitened_stage.ac, psd_dt)
    whitened_psd_c = _positive_period_psd(whitened_stage.c, psd_dt)
    despiked_psd_ac = _positive_period_psd(despiked_stage.ac, psd_dt)
    despiked_psd_c = _positive_period_psd(despiked_stage.c, psd_dt)
    ...
    global_avg_despiked_ac = vec(mean(despiked_stage.ac; dims=2))
    global_avg_despiked_c = vec(mean(despiked_stage.c; dims=2))
    ...
    jldsave(joinpath(run_dir, "source_state_averages.jld2");
        ...
        psd_dt=psd_dt,
        psd_periods=state_psd_ac.periods,
        psd_frequencies=state_psd_ac.frequencies,
        psd_acausal_states=state_psd_ac.psd,
        psd_causal_states=state_psd_c.psd,
        psd_global_ac=vec(global_psd_ac.psd),
        psd_global_c=vec(global_psd_c.psd),
        psd_window_mean_ac=vec(mean(window_psd_ac.psd; dims=2)),
        psd_window_mean_c=vec(mean(window_psd_c.psd; dims=2)),
        psd_state_window_mean_ac=state_window_psd_ac.psd,
        psd_state_window_mean_c=state_window_psd_c.psd,
        psd_state_window_counts_ac=state_window_psd_ac.counts,
        psd_state_window_counts_c=state_window_psd_c.counts,
        preprocessing_stage_names=String.(["raw", "whitened", "despiked"]),
        ...
        global_avg_despiked_ac=Float32.(global_avg_despiked_ac),
        global_avg_despiked_c=Float32.(global_avg_despiked_c),
        psd_window_mean_raw_ac=vec(mean(raw_psd_ac.psd; dims=2)),
        psd_window_mean_raw_c=vec(mean(raw_psd_c.psd; dims=2)),
        psd_window_mean_whitened_ac=vec(mean(whitened_psd_ac.psd; dims=2)),
        psd_window_mean_whitened_c=vec(mean(whitened_psd_c.psd; dims=2)),
        psd_window_mean_despiked_ac=vec(mean(despiked_psd_ac.psd; dims=2)),
        psd_window_mean_despiked_c=vec(mean(despiked_psd_c.psd; dims=2)),
        psd_global_raw_ac=vec(_positive_period_psd(global_avg_raw_ac, psd_dt).psd),
        psd_global_raw_c=vec(_positive_period_psd(global_avg_raw_c, psd_dt).psd),
        psd_global_whitened_ac=vec(_positive_period_psd(global_avg_whitened_ac, psd_dt).psd),
        psd_global_whitened_c=vec(_positive_period_psd(global_avg_whitened_c, psd_dt).psd),
        psd_global_despiked_ac=vec(_positive_period_psd(global_avg_despiked_ac, psd_dt).psd),
        psd_global_despiked_c=vec(_positive_period_psd(global_avg_despiked_c, psd_dt).psd),
        whitening_fir=hasproperty(data_bundle, :whitening_fir) ? Float32.(data_bundle.whitening_fir) : Float32[],
