...
- **Slot attention pooling** gives each slot code a learned temporal focus
...
### Architecture
```
waveform (nt,) → ConvEncoder → z_map (L, d) → slot attention → z_e (d, T) → VQ → z_q (d, T) → ConvDecoder → x̂ (nt,)
                                                    ↓
                                             codebook indices (T,)
```
"""
...
begin
	...
	function softmax_time(x::AbstractArray{Float32,3})
	    NNlib.softmax(x; dims=1)
	end
end
...
Base.@kwdef struct VQVAE_Para
    ...
    attention_entropy_weight::Float32 = 0.0f0  # encourages sharper slot attention
    attention_diversity_weight::Float32 = 0.01f0  # penalizes overlap between slot attention maps
    use_slot_attention::Bool = true       # if false, use direct flattening into slot vectors
...
begin
    ...
    function (m::SeqConv1DChain)(x)
        ...
        y = m.chain(x3)
        return ndims(y) == 3 ? y : reshape(y, size(y, 1), 1, size(y, 2))
    end
end
...
begin
    struct VQVAE{E,P,A,VQ,D,SW}
        encoder::E         # SeqConv1DChain: waveform → feature map (L, C, B)
        pre_vq::P          # 1x1 Conv or Dense: feature map/flat → latent map or slot vectors
        attention_head::A  # 1x1 Conv or nothing; latent map → slot logits (L, T, B)
        quantizer::VQ      # Vector{VectorQuantizerEMA}, one codebook per slot
        decoder::D         # Conv1DChain: (d*T or d) → waveform
        slot_weighter::SW  # Dense(d*T → T) or nothing; produces softmax slot weights from pooled pre-VQ latents
        T::Int             # number of quantized vectors per waveform
        d::Int             # codebook embedding dimension
        superposition::Bool  # if true, decode T slots independently (d each) then average
        use_slot_attention::Bool  # if false, use direct flattening path
    end
    Flux.@layer VQVAE trainable = (encoder, pre_vq, attention_head, decoder, slot_weighter)

    codebook_size(m) = m.quantizer[1].K

    function direct_slot_latents(m, feat)
        z_pre = m.pre_vq(feat)
        if ndims(z_pre) == 2
            N_total = size(z_pre, 2)
            return reshape(z_pre, m.d, m.T, N_total), z_pre, nothing, nothing, 0f0, 0f0
        elseif ndims(z_pre) == 3
            N_total = size(z_pre, 3)
            return z_pre, reshape(z_pre, m.d * m.T, N_total), nothing, nothing, 0f0, 0f0
        else
            error("Unsupported direct slot latent shape")
        end
    end

    function pool_slots(m, z_map::AbstractArray{Float32,3})
        attn_logits = m.attention_head(z_map)                   # (L, T, B)
        attn_weights = softmax_time(attn_logits)                # (L, T, B)

        _, _, N_total = size(z_map)
        z_map_bt = NNlib.batched_transpose(z_map)               # (d, L, B)
        attn_bt = NNlib.batched_transpose(attn_weights)         # (T, L, B)

        # Batched pooled slot latents: (d,L,B) * (L,T,B) -> (d,T,B)
        slot_latents = NNlib.batched_mul(z_map_bt, attn_weights)

        attn_entropy = -sum(attn_weights .* log.(clamp.(attn_weights, 1f-10, 1f0))) /
                       max(Float32(m.T * N_total), 1f0)
        overlap_batch = NNlib.batched_mul(attn_bt, attn_weights)  # (T,T,B)
        overlap = dropdims(sum(overlap_batch; dims=3); dims=3)
        attention_diversity_loss = (sum(overlap) - sum(diag(overlap))) /
                                   max(Float32(N_total * max(m.T * (m.T - 1), 1)), 1f0)

        return slot_latents, attn_logits, attn_weights, attn_entropy, attention_diversity_loss
    end
    ...
    function decode_from_latents(m, result)
        ...
        xhat, xhat_per_slot = if m.superposition
            # Decode each VQ slot independently: decoder sees (d, T*N_total)
            xhat_flat = m.decoder(result.z_q_flat)                      # (nt, T*N_total)
            xhat_slots = reshape(xhat_flat, :, m.T, N_total)            # (nt, T, batch)
            if m.slot_weighter !== nothing
                w = softmax(m.slot_weighter(result.z_pre_flat); dims=1) # (T, batch)
                w_3d = reshape(w, 1, m.T, N_total)                      # (1, T, batch)
                dropdims(sum(xhat_slots .* w_3d; dims=2); dims=2), xhat_slots
            else
                dropdims(mean(xhat_slots; dims=2); dims=2), xhat_slots  # (nt, batch), (nt, T, batch)
            end
        else
            z_q_for_dec = reshape(result.z_q, m.d * m.T, N_total)
            m.decoder(z_q_for_dec), nothing
        end
    ...
    function (m::VQVAE)(x; beta_commit::Float32=0.25f0, training::Bool=true)
        return decode_from_latents(m, encode(m, x; beta_commit=beta_commit, training=training))
    end
    ...
    No decoder pass.
    """
    function encode(m, x; beta_commit::Float32=0.25f0, training::Bool=false)
        x_flat = flatten_batch(x)
        feat_map = m.encoder(x_flat)
        z_map = m.use_slot_attention ? m.pre_vq(feat_map) : nothing
        if m.use_slot_attention
            slot_latents, attn_logits, attn_weights, attn_entropy, attention_diversity_loss =
                pool_slots(m, z_map)
            N_total = size(slot_latents, 3)
            z_pre_flat = reshape(slot_latents, m.d * m.T, N_total)
        else
            slot_latents, z_pre_flat, attn_logits, attn_weights, attn_entropy, attention_diversity_loss =
                direct_slot_latents(m, feat_map)
            N_total = size(slot_latents, 3)
        end
        ...
        for t in 1:m.T
            rt = m.quantizer[t](view(z_e, :, t, :); beta_commit, training)
            z_q_flat = isnothing(z_q_flat) ? rt.z_q : cat(z_q_flat, rt.z_q; dims=2)
            if !training
        ...
        z_q = reshape(z_q_flat, m.d, m.T, N_total)
        slot_scale = max(Float32(m.T), 1f0)
        return (; z_map, z_pre_flat, z_e=reshape(z_e, m.d, m.T, N_total), z_q,
                  z_q_flat, codebook_indices,
                  attention_logits=attn_logits, attention_weights=attn_weights,
                  vq_loss = vq_loss_total / slot_scale,
                  commit_loss = commit_loss_total / slot_scale,
                  perplexity = perplexity_sum / m.T,
                  entropy_loss = entropy_loss_total / slot_scale,
                  attention_entropy=attn_entropy, attention_diversity_loss)
    end
...
function get_vqvae(para)
    ...
    if para.use_slot_attention
        # Encoder preserves latent time axis for slot attention.
        ...
        attention_head = Chain(Conv((1,), para.d => para.T; stride=1, pad=0)) |> xpu
    else
        # Direct-flatten fallback: old v3-style path, no slot attention.
        encoder, flat_len = get_vq_conv_encoder(para.nt;
            kernels=para.enc_kernels, filters=para.enc_filters,
            strides=para.enc_strides, use_bn=para.use_bn,
            flatten_output=true, return_outsize=false)
        pre_vq = Dense(flat_len, para.d * para.T) |> xpu
        attention_head = nothing
    end

    # Separate Vector Quantizer (EMA) per slot.
    quantizer = [VectorQuantizerEMA(para.K, para.d;
        decay=para.ema_decay, epsilon=para.epsilon,
        dead_threshold=para.dead_threshold)
        for _ in 1:para.T]

    # Decoder: (d*T) → nt in standard mode, or (d) → nt in superposition mode
    d_in_dec = para.decode_codebook_superposition ? para.d : para.d * para.T
    ...
    # Slot weighter: only built when both superposition and use_slot_weights are true
    slot_weighter = (para.decode_codebook_superposition && para.use_slot_weights) ?
        xpu(Dense(para.d * para.T, para.T)) : nothing

    model = VQVAE(xpu(encoder), xpu(pre_vq), attention_head === nothing ? nothing : xpu(attention_head),
                  quantizer, xpu(decoder), slot_weighter,
                  para.T, para.d, para.decode_codebook_superposition, para.use_slot_attention)
    ...
    loss_history = (;
        ...
        train_attention_entropy = Float32[],
        test_attention_entropy = Float32[],
        train_attention_diversity = Float32[],
        test_attention_diversity = Float32[],
    )
...
function loss_vqvae(model, x, para; training::Bool=true)
    ...
    attention_diversity_loss = result.attention_diversity_loss

    total = recon_loss + commit_loss +
            para.entropy_weight * entropy_loss -
            para.attention_entropy_weight * attention_entropy +
            para.attention_diversity_weight * attention_diversity_loss

    return (; total, recon_loss, commit_loss, entropy_loss,
              attention_entropy, attention_diversity_loss,
...
function update(model, loss_history, D_train, D_test,
    ...
    @progress name = "VQ-VAE training" for epoch = 1:training_para.nepoch
        ...
        push!(loss_history.train_attention_entropy, train_metrics.attention_entropy)
        push!(loss_history.test_attention_entropy, test_metrics.attention_entropy)
        push!(loss_history.train_attention_diversity, train_metrics.attention_diversity_loss)
        push!(loss_history.test_attention_diversity, test_metrics.attention_diversity_loss)
