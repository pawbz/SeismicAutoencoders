...
"""
has_conditioning_local(conditioning) = conditioning !== nothing
...
begin
    ...
    bench_Dtrain    = [rand(Float32, bench_nt, bench_ntr) for _ in 1:bench_ngroups]
end
...
let
    # warm-up
    get_data_iterator_baseline(bench_Dtrain; ntau=bench_ntau,
        batchsize=bench_batchsize, nsteps=bench_nsteps)
    # timed run (minimum wall-clock seconds)
    t = @belapsed get_data_iterator_baseline($bench_Dtrain; ntau=$bench_ntau,
        batchsize=$bench_batchsize, nsteps=$bench_nsteps)
    nsamples = bench_nsteps * bench_batchsize
	t
    # md"""
    # **Current** `get_data_iterator`: **$(round(t*1000; digits=1)) ms**
    # (nsamples=$(nsamples), ntau=$(bench_ntau), nt=$(bench_nt),
    #  ngroups=$(bench_ngroups), threads=$(Threads.nthreads()))
    # """
end
...
let
    # warm-up
    get_data_iterator_fast(bench_Dtrain; ntau=bench_ntau,
        batchsize=bench_batchsize, nsteps=bench_nsteps)
    # timed run (minimum wall-clock seconds)
    t = @belapsed get_data_iterator_fast($bench_Dtrain; ntau=$bench_ntau,
        batchsize=$bench_batchsize, nsteps=$bench_nsteps)
    nsamples = bench_nsteps * bench_batchsize
	t
    # md"""
    # **Fast** `get_data_iterator_fast`: **$(round(t*1000; digits=1)) ms**
    # (nsamples=$(nsamples), ntau=$(bench_ntau), nt=$(bench_nt),
    #  ngroups=$(bench_ngroups), threads=$(Threads.nthreads()))
    # """
end
...
md"### Correctness: shapes & eltypes match between fast and current"
...
let
    ...
    @assert size(b_orig) == size(b_fast) "shape mismatch: $(size(b_orig)) vs $(size(b_fast))"
    @assert eltype(b_orig) == eltype(b_fast) "eltype mismatch"
    md"✅ shapes match: $(size(b_fast)), eltype $(eltype(b_fast))"
end
...
[deps]
BenchmarkTools = "6e4b80f9-93f8-5d43-8f7f-35e3d6c12f8d"
...
manifest_format = "2.0"
project_hash = "589e709c4ab3470421873fbac9ad7bbb4e5d9808"
