...
"""
Base.@kwdef struct SeismicTrace
    data::Vector{Float64}  
...
function perform_mft_analysis(trace::SeismicTrace, periods::Vector{Float64};
    ...
    data_unpadded = resample(trace.data, 10.0)
    dt            = trace.dt / 10.0
    ...
    time             = collect(range(0.0, step=dt, length=npts_original))
...
function plot_multipeak_dispersion(modes::Vector{MultimodalDispersion}; 
    ...
    colors = PlotlyBase.palette(colorscale, n_modes)
...
"""
    zero_lag_correlation(envelope_causal::Matrix{Float64}, envelope_acausal::Matrix{Float64})

Compute normalized zero-lag cross-correlation between causal and acausal envelopes at each frequency.

**Purpose:** Validates consistency between causal and acausal branches of ambient noise correlations.
High correlation (>0.85) indicates reliable, physically consistent arrivals across both branches.

# Arguments
- `envelope_causal` : Amplitude envelopes [time × frequency] from causal branch
- `envelope_acausal` : Amplitude envelopes [time × frequency] from acausal branch

# Returns
- `Vector{Float64}` — Correlation coefficient [0,1] at each frequency (period)
"""
function zero_lag_correlation(envelope_causal::Matrix{Float64}, envelope_acausal::Matrix{Float64})
    nfreq = size(envelope_causal, 2)
    correlations = Float64[]
    
    for i in 1:nfreq
        ec = envelope_causal[:, i]
        ea = envelope_acausal[:, i]
        ...
        # Compute correlation
        numerator = sum(ec_norm .* ea_norm)
        denominator = sqrt(sum(ec_norm.^2) * sum(ea_norm.^2))
...
"""
    analyze_causal_acausal_branches(trace_causal::SeismicTrace, trace_acausal::SeismicTrace,
                                   ...
                                   kwargs...) -> BranchAnalysisResult

Perform MFT analysis on causal and acausal branches of ambient noise cross-correlations.

**Workflow:**
1. Run MFT independently on causal and acausal traces
2. Extract multipeak modes from each branch
3. Optionally compute zero-lag correlation to validate branch consistency
4. Return combined result structure for downstream analysis/comparison

**Use case:** Ambient noise cross-correlation functions (CCF) have two symmetric branches: positive time lags 
(causal, forward propagation) and negative time lags (acausal, time-reversed). This function enables:
- Testing for mode consistency between branches (physical validity check)
- Averaging or selecting most reliable modes
- Detecting multimode/overtone complexity that appears on both branches

# Arguments
- `trace_causal` : SeismicTrace from causal branch (positive time lags)
- `trace_acausal` : SeismicTrace from acausal branch (time-reversed negative lags)
- `periods` : Analysis periods [s]
- `distance` : Source-receiver distance [km] (used for group velocity computation)
- `max_modes` : Maximum modes to extract per branch (default: 4)
- `compute_correlation` : If true, compute zero-lag envelope correlation (default: true)
- `kwargs...` : Additional arguments passed to perform_mft_analysis (bandwidth_factor, zero_pad_factor, etc.)

# Returns
- `BranchAnalysisResult` — Combined analysis with both MFT results, extracted modes, and branch correlation metrics

# Example
```julia
# Split ambient noise CCF into causal and acausal branches
ccf = read_ccf("station_pair.h5")
causal_branch = ccf[:div(end,2)]
acausal_branch = reverse(ccf[div(end,2):end])

trace_c = SeismicTrace(data=causal_branch, dt=0.01, distance=150.0)
trace_a = SeismicTrace(data=acausal_branch, dt=0.01, distance=150.0)

periods = collect(5:1:50)
result = analyze_causal_acausal_branches(trace_c, trace_a, periods; distance=150.0)

# Compare modes
println("Causal modes: \$(length(result.causal_modes))")
println("Acausal modes: \$(length(result.acausal_modes))")

# Check branch consistency (correlation > 0.85 is good)
plot(result.periods, result.branch_correlation; 
...
function analyze_causal_acausal_branches(trace_causal::SeismicTrace, 
    ...
    modes_causal = extract_all_modes(result_causal; distance=distance, max_modes=max_modes)
    modes_acausal = extract_all_modes(result_acausal; distance=distance, max_modes=max_modes)
    
    # Compute branch correlation if requested
    correlations = fill(NaN, length(periods))
    if compute_correlation
        correlations = zero_lag_correlation(result_causal.envelopes, result_acausal.envelopes)
    end
    
    return BranchAnalysisResult(result_causal, result_acausal, 
                               modes_causal, modes_acausal,
                               correlations, periods, distance)
end
...
function plot_branch_comparison(result::BranchAnalysisResult; 
    ...
    colors = PlotlyBase.palette(colorscale, max(n_max_modes, 2))
...
manifest_format = "2.0"
project_hash = "5bad64d8d30de723e2a4b304d4c0cdbe370f9358"
