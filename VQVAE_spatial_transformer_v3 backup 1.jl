...
**VQ-VAE + fixed discrete shift codebook** for jointly learning time-shift
alignment and discrete waveform clustering.

### Key Ideas
- **Fixed discrete Fourier shifts**: a shift predictor selects from a physically valid
  set of time-shift states; the selected shift is applied via Fourier interpolation
...
- **Fixed discrete shift codebook**: the model selects among a physically valid
  set of time shifts instead of updating shift prototypes with EMA
...
1. As the codebook forms, the shift network learns to align waveforms to a discrete
   physically valid alignment state.
2. Gradients flow end-to-end through both the Fourier shift and the VQ-VAE encoder.
3. The factorization into shape code and shift code can make noisy alignment more
   stable and easier to inspect.

### Architecture
```
x (nt,B) ──→ ShiftCodebookNet ──→ τ (1,B)
      │                                │
...
Base.@kwdef struct VQVAE_Para
    ...
    K::Int = 8                             # legacy coarse codebook alias
    Ksmall::Int = 8                        # coarse codebook size (top level)
    Klarge::Int = 64                       # detail codebook size (residual levels)
    detail_stages::Int = 1                 # number of residual RVQ stages
    coarse_pool_mode::Symbol = :mean       # :mean or :max over latent time slots
    # Optional V4-style nearby-waveform smoothing before coarse quantization
    smooth_cos_threshold::Float32 = 0.8f0
    smooth_max_neighbors::Int = 16
    smooth_min_neighbors::Int = 4
    smooth_temperature::Float32 = 0.1f0
    smooth_blend::Float32 = 0.0f0          # 0.0 disables smoothing, 1.0 fully smoothed
    ...
    shift_temperature::Float32 = 1f0     # softmax temperature for shift selection
    shift_codebook_size::Int = 33        # number of discrete shift states (odd so zero shift is included)
    mean_abs_shift_gamma::Float32 = 0f0  # soft penalty on batch mean |shift|
...
The v2 model does not use a physics-based latent window.
"""
...
begin
    ...
    """
    Shift codebook predictor:
    - waveform branch: `(nt, B) -> shift logits`
    - optional condition branch: `(condition_dim, B) -> shift logits`

    The network predicts logits over a fixed, physically valid shift codebook.
    During the forward pass we use a straight-through hard selection:
    the selected shift is discrete in the forward pass, while gradients flow
    through the softmax probabilities underneath.
    """
    struct ShiftCodebookNet{W,H,C,CH,S}
        wave_chain::W
        wave_head::H
        cond_chain::C
        cond_head::CH
    ...
    Flux.@layer ShiftCodebookNet trainable = (wave_chain, wave_head, cond_chain, cond_head)

    function (m::ShiftCodebookNet)(x::AbstractMatrix{Float32}, condition=nothing)
        # x: (nt, B)
        ...
        if m.shift_input_mode in (:waveform, :both)
            ...
            wave_logits = m.wave_head(wave_feat)    # (S, B)
            raw_logits = wave_logits
        end
        ...
        if m.shift_input_mode in (:condition, :both)
            ...
            cond_logits = m.cond_head(cond_feat)
            raw_logits = isnothing(raw_logits) ? cond_logits : raw_logits .+ cond_logits
        end

        isnothing(raw_logits) && error("shift_input_mode=$(m.shift_input_mode) selected no inputs")

        logits = raw_logits ./ max(m.temperature, 1f-6)
        probs = Flux.softmax(logits; dims=1)
        hard_idx = [Int(argmax(cpu(@view probs[:, i]))) for i in 1:size(probs, 2)]
        hard = xpu(Float32.(Flux.onehotbatch(hard_idx, 1:length(m.shift_codebook))))
        st = hard .+ probs .- Zygote.dropgrad(probs)
        shifts = reshape(m.shift_codebook, 1, :) * st
        return (; shifts, shift_logits=logits, shift_probs=probs,
            shift_hard=hard, shift_index=hard_idx)
    end

    """
    Build shift-codebook predictor with an optional condition branch.
    If `condition_dim == 0`, conditioning is disabled.
    """
    function build_shift_codebook_net(nt::Int, max_shift::Int;
        kernels=[32, 16, 8], filters=[8, 16, 32],
        condition_dim::Int=0, condition_hidden::Int=32,
        shift_input_mode::Symbol=:both, shift_codebook_size::Int=33,
        shift_temperature::Float32=1f0)
        ...
        shift_codebook_size < 2 &&
            error("shift_codebook_size must be ≥ 2")
        layers = Any[]
        wave_chain = nothing
        wave_head = nothing
        if shift_input_mode in (:waveform, :both)
            ...
            wave_head = Dense(nin, shift_codebook_size)
            empty!(layers)
        end
        ...
        if shift_input_mode in (:condition, :both)
            ...
            cond_head = Dense(condition_hidden, shift_codebook_size)
        end

        shift_codebook = xpu(collect(range(-Float32(max_shift), Float32(max_shift);
            length=shift_codebook_size)))
        return ShiftCodebookNet(wave_chain, wave_head, cond_chain, cond_head,
            shift_codebook, Float32(shift_temperature), condition_dim, shift_input_mode)
    end
end
...
begin
    ...
    Forward pass:
      1. ShiftCodebookNet(x) → τ   (hard shift from fixed discrete states)
    ...
    struct VQVAE_ST{VQ, LN, G}
        ...
        locnet::LN              # ShiftCodebookNet: x → hard shift
    ...
    function (m::VQVAE_ST)(x; beta_commit::Float32=0.25f0, training::Bool=true, condition=nothing)
        ...
        shift_result = m.locnet(x_flat, condition_dev)
        τ = shift_result.shifts                               # (1, B) in samples
        ...
        return merge(vq_result, (; xhat, xhat_shifted, shifts=τ), shift_result)
    end
...
# ╔═╡ 4c1a5ed7-1f39-4b12-97e4-1f7f5b3d8c21
begin
    struct ResidualShiftCodebookNet{C,H,S}
        chain::C
        head::H
        shift_codebook::S
        temperature::Float32
    end
    Flux.@layer ResidualShiftCodebookNet trainable = (chain, head)

    function (m::ResidualShiftCodebookNet)(residual_latent::AbstractMatrix{Float32}, condition=nothing)
        feat = m.chain(residual_latent)
        logits = m.head(feat) ./ max(m.temperature, 1f-6)
        probs = Flux.softmax(logits; dims=1)
        hard_idx = [Int(argmax(cpu(@view probs[:, i]))) for i in 1:size(probs, 2)]
        hard = xpu(Float32.(Flux.onehotbatch(hard_idx, 1:length(m.shift_codebook))))
        st = hard .+ probs .- Zygote.dropgrad(probs)
    ...
    function build_residual_shift_codebook_net(d::Int, max_shift::Int;
        hidden::Int=64, shift_codebook_size::Int=33, shift_temperature::Float32=1f0)
        shift_codebook = xpu(collect(range(-Float32(max_shift), Float32(max_shift);
            length=shift_codebook_size)))
        chain = Chain(
            Dense(d, hidden, activation),
            Dense(hidden, hidden, activation),
        )
        head = Dense(hidden, shift_codebook_size)
        return ResidualShiftCodebookNet(chain, head, shift_codebook, Float32(shift_temperature))
    end

    struct VQVAE_V3{E,P,QC,QD,SN,D,G}
        encoder::E
        slot_projector::P
        coarse_quantizer::QC
        detail_quantizers::QD
        shift_net::SN
        decoder::D
        sampling_grid::G
        T::Int
        d::Int
        detail_stages::Int
        coarse_pool_mode::Symbol
        smooth_cos_threshold::Float32
        smooth_max_neighbors::Int
        smooth_min_neighbors::Int
        smooth_temperature::Float32
        smooth_blend::Float32
    end
    Flux.@layer VQVAE_V3 trainable = (encoder, slot_projector, shift_net, decoder)

    codebook_size(m::VQVAE_V3) = m.coarse_quantizer.K
    get_codebook(m::VQVAE_V3) = cpu(m.coarse_quantizer.embedding)

    function pool_latent_slots(slot_latents::AbstractArray{Float32,3}, mode::Symbol)
        if mode == :mean
            return dropdims(mean(slot_latents; dims=2); dims=2)
        elseif mode == :max
            return dropdims(maximum(slot_latents; dims=2); dims=2)
        else
            error("coarse_pool_mode must be :mean or :max, got $mode")
        end
    end

    function cosine_neighbor_weights(z_e::AbstractMatrix{Float32};
        threshold::Float32,
        max_neighbors::Int,
        min_neighbors::Int,
        temperature::Float32)
        ...
        norms = sqrt.(sum(abs2, z_cpu; dims=1)) .+ 1f-8
        z_norm = z_cpu ./ norms
        S = z_norm' * z_norm

        thr = clamp(threshold, -1f0, 1f0)
        temp = max(temperature, 1f-4)
        kmin = min(min_neighbors, B - 1)
        kcap = min(max_neighbors, B - 1)
        ...
        for j in 1:B
            ...
            for i in 1:B
                i == j && continue
                sims[i] >= thr && push!(cand, i)
            end

            if length(cand) < kmin
                order = sortperm(collect(sims); rev=true)
                empty!(cand)
                for i in order
                    i == j && continue
                    push!(cand, i)
                    length(cand) >= kmin && break
                end
            else
                cand = sort(cand; by=i -> sims[i], rev=true)
            end
            ...
            vals = Float32[sims[i] for i in cand]
            vmax = maximum(vals)
            ws = exp.((vals .- vmax) ./ temp)
            ws ./= (sum(ws) + 1f-8)
    ...
    function encode(m::VQVAE_V3, x; beta_commit::Float32=0.25f0, training::Bool=false, condition=nothing)
        x_flat = xpu(flatten_batch(x))
        feat_map = m.encoder(x_flat) # (L, C, B)
        ndims(feat_map) == 3 || error("V3 encoder must output (L, C, B), got ndims=$(ndims(feat_map))")

        L, C, B = size(feat_map)
        feat_flat = reshape(feat_map, C, L * B)
        slot_flat = m.slot_projector(feat_flat)
        slot_latents = reshape(slot_flat, m.d, L, B)

        z_coarse_in = pool_latent_slots(slot_latents, m.coarse_pool_mode) # (d, B)
        if m.smooth_blend > 0f0 && B > 1
            W = Zygote.@ignore cosine_neighbor_weights(z_coarse_in;
                threshold=m.smooth_cos_threshold,
                max_neighbors=m.smooth_max_neighbors,
                min_neighbors=m.smooth_min_neighbors,
                temperature=m.smooth_temperature)
            z_smooth = z_coarse_in * W
            z_coarse_in = m.smooth_blend .* z_smooth .+ (1f0 - m.smooth_blend) .* z_coarse_in
        end
        rt_coarse = m.coarse_quantizer(z_coarse_in; beta_commit, training)

        z_coarse = rt_coarse.z_q
        z_coarse_slots = reshape(z_coarse, m.d, 1, B)
        residual_slots = slot_latents .- z_coarse_slots
        detail_sum_slots = zeros(Float32, size(residual_slots)) |> xpu

        detail_commit = 0f0
        detail_entropy = 0f0
        detail_vq = 0f0
        detail_perplexity_acc = 0f0
        detail_idx = training ? nothing : zeros(Int, m.detail_stages, L, B)

        for (s, dq) in enumerate(m.detail_quantizers)
            r_flat = reshape(residual_slots .- detail_sum_slots, m.d, L * B)
            rt = dq(r_flat; beta_commit, training)
            dq_slots = reshape(rt.z_q, m.d, L, B)
            detail_sum_slots .+= dq_slots
            detail_commit += rt.commit_loss
            detail_entropy += rt.entropy_loss
            detail_vq += rt.vq_loss
            detail_perplexity_acc += rt.perplexity
            if !training
                detail_idx[s, :, :] .= reshape(Int.(cpu(rt.indices)), L, B)
            end
        end

        z_canonical_slots = z_coarse_slots .+ detail_sum_slots
        z_decode_flat = reshape(z_canonical_slots, m.d * L, B)
        xhat_canonical = m.decoder(z_decode_flat)

        shift_input = reshape(residual_slots, m.d * L, B)
        shift_result = m.shift_net(shift_input, condition)
        τ = shift_result.shifts

        xhat = shift_traces_Fourier(xhat_canonical, τ, m.sampling_grid)
        x_shifted = shift_traces_Fourier(x_flat, -τ, m.sampling_grid)

        coarse_idx = training ? nothing : reshape(Int.(cpu(rt_coarse.indices)), 1, B)
        mean_detail_perplexity = m.detail_stages > 0 ? detail_perplexity_acc / m.detail_stages : 1f0

        return merge(shift_result, (; 
            z_map=feat_map,
            z_e=slot_latents,
            z_q=z_canonical_slots,
            z_q_flat=z_decode_flat,
            codebook_indices=coarse_idx,
            coarse_codebook_indices=coarse_idx,
            detail_codebook_indices=detail_idx,
            vq_loss=rt_coarse.vq_loss + detail_vq,
            commit_loss=rt_coarse.commit_loss + detail_commit,
            perplexity=rt_coarse.perplexity,
            detail_perplexity=mean_detail_perplexity,
            entropy_loss=rt_coarse.entropy_loss + detail_entropy,
            x_shifted,
            xhat_shifted=xhat_canonical,
            xhat,
            shifts=τ,
        ))
    end

    function (m::VQVAE_V3)(x; beta_commit::Float32=0.25f0, training::Bool=true, condition=nothing)
        return encode(m, x; beta_commit=beta_commit, training=training, condition=condition)
    end
end
...
function get_vqvae(para)
    ...
    smooth_blend = clamp(para.smooth_blend, 0f0, 1f0)
    para.smooth_max_neighbors >= 1 || error("smooth_max_neighbors must be >= 1")
    para.smooth_min_neighbors >= 1 || error("smooth_min_neighbors must be >= 1")
    ...
    slot_projector = Dense(enc_channels, para.d) |> xpu

    Ksmall = para.Ksmall > 0 ? para.Ksmall : para.K
    coarse_quantizer = VectorQuantizerEMA(Ksmall, para.d;
        decay=para.ema_decay, epsilon=para.epsilon,
        dead_threshold=para.dead_threshold)

    n_detail = max(para.detail_stages, 0)
    detail_quantizers = [VectorQuantizerEMA(para.Klarge, para.d;
        decay=para.ema_decay, epsilon=para.epsilon,
        dead_threshold=para.dead_threshold) for _ in 1:n_detail]

    d_in_dec = para.d * latent_len
    ...
    shift_net = build_residual_shift_codebook_net(para.d * latent_len, para.max_shift_samples;
        hidden=para.condition_hidden,
        shift_codebook_size=para.shift_codebook_size,
        shift_temperature=para.shift_temperature) |> xpu
    ...
    model = VQVAE_V3(xpu(encoder), slot_projector, coarse_quantizer, detail_quantizers,
        shift_net, xpu(decoder), sampling_grid, 1, para.d, n_detail, para.coarse_pool_mode,
        para.smooth_cos_threshold, para.smooth_max_neighbors, para.smooth_min_neighbors,
        para.smooth_temperature, smooth_blend)

    @info "VQVAE_ST V3 geometry" nt=para.nt latent_len enc_outsize dec_upstrides dec_out_len max_shift_samples=para.max_shift_samples shift_codebook_size=para.shift_codebook_size Ksmall=Ksmall Klarge=para.Klarge detail_stages=n_detail coarse_pool_mode=para.coarse_pool_mode smooth_blend=smooth_blend smooth_cos_threshold=para.smooth_cos_threshold
...
VQ-VAE loss for the fixed-shift-codebook model:

    L = L_recon + L_commit + β_entropy·H
"""
function loss_vqvae(model, x, para; training::Bool=true, condition=nothing)
    result = model(x; beta_commit=para.beta_commit, training, condition=condition)

    recon_loss   = Flux.mse(result.xhat, x)
    commit_loss  = result.commit_loss
    entropy_loss = result.entropy_loss
    mean_abs_shift_loss = para.mean_abs_shift_gamma * mean(abs, result.shifts)
    ...
    return (; total, recon_loss, commit_loss, entropy_loss,
        mean_shift, mean_abs_shift_loss,
...
function update(model, loss_history, D_train, D_test, para,
    ...
    @progress name="VQ-VAE ST training" for epoch = 1:training_para.nepoch
        ...
        if mod(epoch, training_para.nprint) == 0
            @info "Epoch $epoch" recon=train_m.recon_loss commit=train_m.commit_loss perplexity=train_m.perplexity mean_shift=train_m.mean_shift test_recon=test_m.recon_loss
        end
